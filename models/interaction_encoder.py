...
class DUORec(nn.Module):
    def __init__(self, item_num, emb_size, max_len, n_layers, n_heads, inner_size, hidden_dropout_prob,
                 attn_dropout_prob, hidden_act, layer_norm_eps, initializer_range):
        super(DUORec, self).__init__()
        self.emb_layer = nn.Embedding(item_num + 2, emb_size, padding_idx=0)
        self.position_embedding = nn.Embedding(max_len, emb_size)
        self.transformer_layers = TransformerEncoder(n_layers=n_layers,
                                                    n_heads=n_heads,
                                                    ...
                                                    eps=initializer_range)
        self.LayerNorm = nn.LayerNorm(emb_size, eps=initializer_range)
    ...
    def forward(self, batch_seqs, sequence_length):
        position_ids = t.arange(batch_seqs.size(1), dtype=t.long, device=batch_seqs.device)
...
class LSTM_interactionEncoder(nn.Module):
    def __init__(self, num_items, embedding_dim, lstm_hidden_dim, num_layers):
        ...
        self.emb_layer = nn.Embedding(num_items + 2, embedding_dim)
    ...
    def forward(self, batch_seqs):
        ...
        lstm_output, (h_n, _) = self.lstm(item_embedded)
